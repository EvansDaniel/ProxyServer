...
/**
 * Processes exactly one request from the client
 * @param connfd represents the connection between the proxy and the client
...
void *process_request(void *connfd) {
  ...
  if (path == NULL) {
    ...
    fprintf(stderr, "path: Invalid uri given: %s", uri);
    cleanup(-1, headers, path, NULL, connfd);
    return NULL;
  }
  // no need to check return b/c rrh checks the input
  char *host = malloc(MAXLINE);
  // add the request header with HTTP/1.0
  sprintf(headers, "%s %s %s\r\n", method, path, "HTTP/1.0");
  if (read_request_headers(&rio, headers, host) == -1) {
    cleanup(-1, headers, path, NULL, connfd);
    return NULL;
  }
  ...
  // prefer host_p (parsed from uri) if not NULL,
  // otherwise set host_p to host (from headers)
  if(host_p == NULL) {
    host_p = host;
  } else {
    // free the host var if host_p was parsed from uri successfully
    free(host);
  }
  if (host_p == NULL) {
      fprintf(stderr, "%s\n", "parse_host failed");
...
int read_request_headers(rio_t *rp, char *headers, char *host) {
  ...
  if(host != NULL) {
    // attempt retrieve the host from the headers
    memset(host, 0, strlen(header));
    memcpy(host, (header + 6), strlen(header) - 6);
